...
const DERIVED_FIELDS: Record<string, string> = {
  ...
  // contacts.
  // ⚠️ `destinations:organizations.name` stood here and is GONE with the field:
...
Deno.test("typesense coverage: every displayDefaults reference names a declared field", () => {
  ...
  for (const [alias, config] of Object.entries(typesenseSchemas)) {
    ...
    // `facet`, `group` and `sort` were checked here too, until all three keys
    // were deleted as write-only (core#50, B4). Worth knowing why this arm did
    // not catch them: it asks whether a reference names a DECLARED FIELD, and
    // `cards.facet`'s `uid_list` named a real one. A reference can be
    // perfectly resolvable and still render nothing, if the thing holding it
    // has no reader.
