...
Deno.test("T10: a bookings rollup actually reaches the column list", () => {
  ...
  // table exists to do. The charge-date rollups this arm used to name were
  // removed with the fields (charge-windows step 5); `organization.name` is the
  // one `bookings` entry left.
  const byKey = new Map(getTypesenseColumns("bookings").map((c) => [c.key, c]));
  assertEquals(byKey.get("organization.name")?.label, "Organization");
  for (const key of ["dates.charge_start_fs", "dates.charge_end_fs"]) {
    assertEquals(byKey.has(key), false, `bookings:${key} was removed with the legacy charge fields`);
  }
});
