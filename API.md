...
```ts
const ActivityChange: z.ZodType<ActivityChangeType>;
```
...
```ts
type ActivityOperation = indexedAccess;
```
...
```ts
const ActivityOperationEnum: z.ZodType<ActivityOperation>;
```
...
```ts
const ActivitySchema: z.ZodType<Activity>;
```
...
```ts
const ActivitySubject: z.ZodType<ActivitySubjectType>;
```

### `ActivitySubjectCollection`

A collection an {@link ActivitySubjectType} can name.

```ts
type ActivitySubjectCollection = indexedAccess;
```
...
```ts
const ActivitySubjectCollectionEnum: z.ZodType<ActivitySubjectCollection>;
```
...
```ts
const ActorKindEnum: z.ZodType<ActorKind>;
```
...
```ts
type CollectionName = keyof CollectionDocs;
```
...
```ts
const CreateDepartmentTypeInput: z.ZodType<CreateDepartmentTypeInputType>;
```

### `CreateDepartmentTypeInputType`

Input type for creating a department type.

```ts
interface CreateDepartmentTypeInputType {
  name: string;
}
```
...
```ts
const DepartmentTypeSchema: z.ZodType<DepartmentType>;
```
...
```ts
const FirestoreDisplayPrefsSchema: z.ZodType<FirestoreDisplayPrefs>;
```
...
```ts
type SchemaDocType = indexedAccess;
```
...
```ts
const UpdateDepartmentTypeInput: z.ZodType<UpdateDepartmentTypeInputType>;
```

### `UpdateDepartmentTypeInputType`
...
```ts
const OrderDerivedOrgPath: z.ZodType<OrgPathNodeType[]>;
```
...
```ts
const INTEGRATION_EVENT_MSGS: "uploadcare_file_not_found" | "uploadcare_orphan_batch_failed" | "uploadcare_orphan_cleanup_failed" | "uploadcare_orphan_sweep_completed" | "uploadcare_upload_abandoned" | "dmarc_report_ingest_failed" | "dmarc_report_processor_run" | "eventarc_duplicate_event" | "eventarc_processed" | "activity_captured" | "activity_skipped" | "trello_locked" | "trello_newer_update_detected" | "trello_no_new_updates" | "trello_queue_error" | "mirror_deleted" | "mirror_failed" | "mirror_set" | "mirror_set_failed_terminal" | "mirror_set_queue_failed" | "mirror_skipped_stale" | "draft_quote_skipped_deleted" | "draft_quote_skipped_invalid_order" | "draft_quote_superseded" | "dns_record_check" | "dns_record_check_resolve_failed" | "location_integrity_check" | "location_integrity_check_failed" | "order_projection_check" | "order_projection_check_failed" | "organization_tree_check" | "organization_tree_check_failed" | "stock_summary_sweep" | "settlement_totals_sweep" | "il_tax_rate_check" | "tax_expiry_check" | "sync_collection_completed" | "sync_collection_skipped" | "sync_started" | "geocode_cache_write_failed" | "geocode_poi_fallback" | "geocoding_failed" | "user_name_cascade_batch" | "customer_linking_failed"[];
```
...
```ts
const OAUTH_EVENT_MSGS: "oauth_401_retry" | "oauth_alert_email_failed" | "oauth_refresh_attempt" | "oauth_refresh_chain_broken" | "oauth_refresh_failed_response" | "oauth_refresh_scheduled" | "oauth_refresh_stale_task" | "oauth_token_exchanged" | "oauth_token_expired_refreshing" | "oauth_token_refresh_skipped" | "oauth_token_refreshed" | "oauth_token_response" | "mcp_oauth_authorize_staged" | "mcp_oauth_client_registered" | "mcp_oauth_code_consume_failed" | "mcp_oauth_consent_approved" | "mcp_oauth_consent_denied" | "mcp_oauth_scope_denied" | "mcp_oauth_token_invalid" | "mcp_oauth_token_minted" | "mcp_oauth_user_missing" | "token_refresh_failed" | "xero_token_exchange_failed"[];
```
...
```ts
interface OrderInvoiceCoverage {
  uninvoiced: LineItem[];
...
```ts
interface UnmatchedInvoiceLine {
  invoiceUid: string;
  item: InvoiceItem;
}
```
...
- `orderUid` — The source order's uid — which IS its divider's uid on every
invoice that bills it (Option B; the transitional `uid_order` field is gone).
- `orderItems` — The order's full `items` array, dividers included — the
...
```ts
type ActionableStage = Exclude<FulfillmentStage, "quoted" | "complete">;
```
...
```ts
const FULFILLMENT_STAGES: "quoted" | "prep" | "checkout" | "return" | "complete"[];
```
...
```ts
type FulfillmentStage = indexedAccess;
```
...
```ts
const STAGE_SOURCE: Record<ActionableStage, BookingBreakdownKeyType>;
```
...
```ts
const STAGE_TARGET: Record<ActionableStage, BookingBreakdownKeyType>;
```
...
### `bucketsForBookingSide(b: Pick<Booking, "type">, stage: FulfillmentStage, side: StageSide): BookingBreakdownKeyType[]`
...
### `bucketsForStageSide(stage: FulfillmentStage, side: StageSide): BookingBreakdownKeyType[]`
...
### `canPrepCheckout(status: indexedAccess | undefined): boolean`
...
### `checkoutUnits(b: Pick<Booking, "breakdown">): number`
...
- `label` — an identifier for the refusal message — a line uid, typically.
...
### `priceTransactionFeeLine(item: PricingItem): LinePriceMoney`
...
```ts
const COLLECTING_JURISDICTIONS: readonly JurisdictionType[];
```
...
### `aggregateGoldenVerdict(verdicts: readonly GoldenDiffVerdict[]): GoldenDiffVerdict | null`
...
### `paragraphAround(text: string, at: number): string`
