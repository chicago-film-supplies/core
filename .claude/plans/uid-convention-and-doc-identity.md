...
- `templates/deno.json` — **exact pins, not a caret**, one per subpath. **Do not carry a count in your head: there is no single right answer.** Measured 2026-08-23, `origin/main` had **7** (`schemas` + `orders`, `templates`, `invoices`, `dates`, `icons`, `money`) while the unmerged `ci/citation-gate` branch had **8** — it adds `utils/citations`. So the number depends on which branch you read, and every previous version of this line stated one as if it were a property of the repo. ⚠️ **Read the version off `origin/main`, not off whatever branch is checked out.** A local `templates` checkout is usually sitting on a draft branch — its branches are user-generated and exempt from hygiene — and that branch can predate the last pin PR. Measured 2026-08-23: the working copy showed `beta.240` while `origin/main` was already at `beta.241` (PR #119, merged 17:14Z), which produced a false "templates is a publish behind" reading in this very document. **Bump by PATTERN, never by the remembered count** — a `sed` over `jsr:@cfs/core@<old>/` cannot miss a line; a remembered number can, and has. The root `~/cfs/CLAUDE.md` records its own copy of this count going stale twice (missing `utils/templates`, then missing `utils/citations`, core#66); this doc was stale by both additions at once, which is how a new subpath gets stranded on the old version.
