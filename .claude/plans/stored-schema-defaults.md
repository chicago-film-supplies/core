...
> ## ⚠️ STATUS 2026-09-11 — **batch 8 is landed, deployed and enforcing in prod. Backlog 90.**
> One statement, compacted rather than stacked. Eight batches. Batch 8 is the **`cards` family's
> pure-storage half — 10 paths across TWO declarations** (`CardSchema` ×5: `all_day`, `attachments`,
> `body_text`, `locked`, `uid_assignees`; `RecurrencePrototype` ×5: `attachments`, `body_text`,
> `locked`, `sources`, `uid_assignees`), taking the backlog **100 → 90**. `core` published
> **`beta.409`** (`5b31d93`); `api-cloudrun` (40 pins, `215d00ff`), `manager` (1 pin, `8e03fb8`,
> released as `manager-v26.0.2`) and `templates` (15 pins, PR #314, merged) are all bumped.
>
> ✅ **The chain is CLOSED, by digest and against the deployed artifact.** Release PR #962 merged as
> `c22b6cf1`, cutting **`v0.249.4`**. That build (`b6654136`, region `us-central1`) produced
> `sha256:e05134d1110fd5776691a7fe3f8885d815b50c56a9a09b317a2a71bb2fd30aff`, and revision
> **`api-cloudrun-00371-zqw`** serves exactly that digest at 100% traffic. The deployed tree's
> `api-cloudrun/deno.json` was diffed against the checkout the probe ran from — **identical, 10,406
> bytes** — so the reparse with **no `--core` flag** parses against the core the image itself
> resolves: **all 5 card positions `REACHED`, 1,161 prod / 1,168 dev documents, 0 failures, exit 0.**
> ⭐ published → pinned → deployed → enforcing, each link a different claim and each one checked.
>
> ### 🔴 The finding: dev was a GENUINE second sample, and it changed the batch
>
> `cards` is **1,159 prod / 1,166 dev** — the first collection since batch 2 where the two corpora
> differ. Those **7 extra dev documents are the only HAND-AUTHORED cards in either corpus**; prod's
> 1,159 are all machine-generated by `buildEventCards`. So "1,159 of 1,159" measures the GENERATOR,
> which is precisely what `api-cloudrun/scripts/audit-field-presence.ts`'s own docblock warns about
> — and this is the first batch where that warning actually fired.
>
> They state all ten keys this batch took. They also **omit `action` (7 of 7) and `organization`
> (6 of 7)** — hand-created April–May 2026 before either field existed, all `status=canceled`. So
> **`cards.action` and `cards.organization` stay in the backlog**: requiring them would put dev's
> `audit:reparse` permanently non-zero, which is the one thing blocking scheduling it
> (api-cloudrun#955). ⚠️ **A prod-only reading would have called both free.** The `organization` set
> is the `action` set minus one document — one population, so one repair, not two.
>
> | position | prod containers/present | dev containers/present | verdict |
> |---|---|---|---|
> | `cards.all_day` · `attachments` · `body_text` · `locked` · `uid_assignees` | 1,159 / 1,159 | 1,166 / 1,166 | ✅ taken |
> | `cards.action` | 1,159 / 1,159 | 1,166 / **1,159** | 🔴 deferred — 7 absent |
...
> rather than a clean `REACHED` and the corpus says nothing at all. Both arms were controlled by
> mutation instead, in an `api-cloudrun` worktree pinned at `file://` this tree:
>
> - **the TYPE** — deleting `sources` from `buildPrototype` fails `TS2741: Property 'sources' is
>   missing … required in type 'RecurrencePrototypeType'`, resolved through the local core.
> - **the SCHEMA** — forcing `sources: undefined` past that type with a cast makes
>   `validateBeforeWrite` refuse at `["prototype","sources"]` and reddens `Recurrences: create
>   materializes horizon of cards`, which POSTs the thinnest legal input there is
>   (`prototype: { subject }`).
>
> 🔴 **So the writer was ALREADY compiler-gated and the schema was the only door left open** — which
> is the general shape of this campaign, stated for once by experiment rather than by reading. An
> empty collection is not an ungateable one; it is one whose instrument is the type and the suite
> that drives its single create author.
>
> ### ⭐ `getInitialValues` is a REAL consumer of these defaults — and the test is decidable
>
> `manager/src/stores/cards.ts` and `manager/src/stores/recurrences.ts` seed new drafts from
> `getInitialValues(CardSchema)` / `(RecurrenceSchema)`. That is a genuine dependency on `.default()`
> that seven previous batches never had to think about. It is inert here for a reason that
> generalises: `resolveField` (`core/src/schemas/initial.ts`) is **TYPE-derived** — `[]` for an
> array, `""` for a string, `false` for a boolean.
>
> 🔴 **Removing `.default(V)` is invisible to the form seed IFF `V` equals the node's type-zero.**
> All ten here do. Measured rather than reasoned: a before/after JSON dump of both initial objects is
> byte-identical, and the probe was mutation-controlled — re-adding `.default(true)` on `all_day`
> moves the seed to `true`. ⚠️ **A default that is NOT the type-zero needs `.meta({ initial })`
> before it is removed**, or the form silently reseeds; that is the case the note in `initial.ts`
> already exists for (five `z.boolean().default(true)` product fields that would have shipped new
> products inactive). **Check this per batch from here on.**
>
> ⚠️ **My first grep for `getInitialValues` MISSED both files** — it was `| head -20` and the card
> stores sorted past the cut. A truncated grep reads exactly like a complete answer. The finding came
> from re-running it unbounded.
>
> ### Deliberately NOT folded in — and the reasons are of two different kinds
>
> - **`CardAttachment.locked`** (`cards.attachments[].locked` + `recurrences.prototype.attachments[].locked`
>   — ONE node, two resolved paths) and **`CardDates.start`/`.end`** are SHARED stored/input nodes:
>   `CreateCardInput`, `UpdateCardInput` and both recurrence inputs reference the same instances.
>   Requiring them is an **API-input change** needing a client-first ordering, not a storage
>   tightening. The other four leaves each have a SEPARATE `.optional()` instance on the inputs,
>   which is the whole reason this batch is pure storage. **A field name is not a node — only
>   grepping the node says which.**
> - **`cards.action` / `cards.organization`** — the 7 dev rows above. A corpus reason, not a
>   structural one.
>
> ⭐ Two exclusions, two unrelated reasons, both stated: that is what keeps a batch reviewable.
>
> ### The gates, and one that must NOT be credited
>
> `core`: **2,285 passed, 0 failed on the first run** — unlike batches 5 and 7, because
> `core/tests/card.test.ts` and `core/tests/recurrence.test.ts` already spell complete documents.
> ⚠️ *Green because complete* and *green because nothing looks* print the same line, so it was
> controlled: a probe dropping each key in turn names exactly that key (`["locked"]`,
> `["prototype.sources"]`, …), and **the two EXCLUDED keys still parse when dropped**.
>
> `api-cloudrun`, pre-publish against `file://` pins: **2,034 passed, 3 failed, all 3 the documented
> guard class** (`lockfileSync`, `referenceCoverage`, `captureFloor`'s *"THIS build resolves a real
> version"*). ⭐ Re-run after the publish against the real `beta.409`: **2,048 passed, 0 failed** —
> those same three green, which is what CONFIRMS the diagnosis rather than merely asserting it. The
> `cards` + `recurrences` integration suites: 57 passed, 0 failed.
>
> 🔴 **`templates` `lint:fixtures` has a ZERO DENOMINATOR here and was not credited.** It reports 38
> fixtures across 7 families and **not one declares `cards` or `recurrences` as its
> `collection_source`** (measured: `orders`, `invoices`, `pick-sheets` ×2, `movement-sessions`,
> `aging-reports`, `statements`). Its green is the absence of a question. Batch 4 learned this on
> `organizations`; it recurs whenever a batch takes a collection templates does not render.
>
> ### ⚠️ A new cross-repo gate landed mid-session and this batch was its first real subject
>
> `api-cloudrun` `6b0cec23` (a peer, same day) rewrote `requires-manager.yaml` to **MEASURE** the
> core pin — this release's `api-cloudrun/deno.json` against the pin `manager`'s latest PUBLISHED release carries
> — instead of asking for a hand-written trailer. It went red on #962 exactly as designed, because
> `manager-v26.0.1` was cut before this batch's manager push. **It is ADVISORY**, and batch 8 is a
> pure storage tightening, so merging anyway was defensible — but a standing red hides the next real
> one, so `manager-v26.0.2` was released FIRST and the gate re-run green. ⭐ That is also the first
> time the gate has passed in its measured polarity on a real release.
>
> ⚠️ **Both release ranges carried exactly one commit — mine.** Checked before merging either, because
> merging a release PR deploys whatever else is sitting on that branch, and with five live sessions
> that is not a safe assumption.
>
> ⚠️ **zsh globbed the `?` in a `gh api …?ref=…` URL again**, exactly as this doc records from batch
> 7. Quote the URL, and check the fetched file's SIZE before diffing it.
>
...
### ✅ `quantity` ×4 + `name` ×4 WAS batch 7, and the `cards` family's pure-storage half WAS batch 8

Batch 7 (`core` `37c4209` → `beta.408`) took 8 paths across four declarations, leaving 100. Batch 8
(`core` `5b31d93` → `beta.409`) took 10 across two, leaving **90**. Both write-ups are in the status
block; what follows is only what they leave behind.

**RE-DERIVED 2026-09-11 AFTER batch 8, by the recipe below — do not quote it, re-run it:**
**90 paths — 66 scalar, 24 `array[]`, 14 of them `items[]`.**

🔴 **`items[]` is `path` ×10 plus four credit-note-only keys, and NOTHING ELSE.** Unchanged by
batch 8, which took no `items[]` path:
...
**The per-FAMILY split, re-derived 2026-09-11 after batch 8:**

| family | paths | note |
|---|---:|---|
| **`items[]` across orders / invoices / credit-notes / fulfillments** | **14** | `path` ×10 + 4 credit-note-only keys; the cheap leaves are gone |
| `products` / `webshop-products` (`webshop.*`, `component_of[]`, `components[]`) | 15 | 🔴 **the largest family left** — two collections, one shared `component_of` block |
...
| the `cards` family's REMAINDER | 5 | see below — two blocked by dev rows, two shared input nodes, one shared `CardAttachment` node |
...
⚠️ **The per-COLLECTION split, re-derived 2026-09-11 after batch 8** (`credit-notes` 9 · `products` 8
· `orders` 8 · `invoices` 8 · `webshop-products` 7 · `transactions` 5 · `fulfillments` 5 · `cards` 5 ·
`templates` 4 · then a tail of 1-3 across ~19 more collections). The FAMILY table above groups these
across collections, which is the unit a batch is chosen in; the two do not add up on purpose.

### 🔴 What batch 8 left inside the `cards` family, and why — two different reasons

**Five paths remain on `cards`, and they are NOT one leftover pile.** Keeping the two reasons
separate is what makes the remainder legible to whoever takes it:
...
⚠️ Note `cards.dates.*` are two of the five and are *scalar*, so the raw `cards` count of 5 spans
both reasons. **Do not take the two structural pairs as a "cards batch"** — they are an input change
that happens to live in the same file, and `CardAttachment` is embedded by `recurrences` too.
...
**Batch 8 has NO residue** — published, swept, merged, deployed, digest-verified and re-confirmed
...
four different claims. Batches 7 and 8 each ran all four and they all held.
...
**Clear before batch 9.**

Batch 8 is closed — nothing mechanical is left to watch. **Batch 9 depends on none of this session's
analysis.** Everything it requires is written down: the backlog is read off
`core/tests/stored-defaults.test.ts` (**90** paths — 66 scalar, 24 `array[]`, 14 of them `items[]`),
...
and batch 8's *a VACUOUS declaration is gated by the TYPE and the SUITE* — is in `core/CLAUDE.md`
§ *`.default()` and `.optional()`*.

⚠️ **Do not carry this doc's numbers into batch 9 — re-run the recipe.** Batch 3 proved the doc's own
...
guard class can be three files rather than two the first time anyone counts it; **batch 8 proved the
doc's own "dev is not a second sample" habit had become an assumption — `cards` is 1,159 / 1,166 and
the 7-document difference decided which paths the batch could take.** Re-derive, and ask what the
instrument can see.

⭐ **Batch 8's transferable habits**, both cheap:
- **Run the consumer suite BEFORE the publish** (batch 7's habit, repeated and again worth it) — and
  then run it AGAIN after, against the real pin. The three `file://` guard-class failures going
  green on the real version is what CONFIRMS the diagnosis rather than merely asserting it.
- **Check `getInitialValues` per batch.** It is a live consumer of these defaults in two manager
  stores, and whether a removal is visible to it is decidable from the node's type-zero.
...
- The two documents `api-cloudrun#951` names are **still unrepaired** — a prod `transactions`
  ...
  reads. The 7 dev cards above are now a fourth item on that same list.
