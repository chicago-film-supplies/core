...
**Date:** 2026-09-16 • **Repo:** core (+ api-cloudrun, manager, templates) • **Status:** in-progress (steps 1–4 + core#114 in prod; step 5 publish built and unreleased — api-cloudrun#1038)
**Origin:** hotspot rentals billed only for their activation windows; owner design sessions 2026-09-16
**Related:** api-cloudrun#1038 (step 5 remainder), api-cloudrun#1028, core#112 (§3 comparator), core#113 (pair invariant guard), templates#376 (partials read windows), core#114 (closed: multi-window lines store billable days)

## START HERE
**Steps 1–4 and core#114 are in prod. Step 5's first publish is built and unreleased.** `@cfs/core@10.0.0-beta.484` (core `5bb03f7`) makes the legacy charge fields optional, stops every writer producing them and drops them from the Typesense configs; manager `032f90d8` and api-cloudrun `fa1688d4` are pinned to it **on `main`, not released**. **Next: release manager, THEN the API** (readers first), then the purge (dev → prod) + prod Typesense resync, then beta C deletes the declarations. The whole remainder is **api-cloudrun#1038**, which lists the fixtures beta C will redden and the two open questions. templates PR #381 (multi-window fixture) still needs its golden approved. First command: `gh issue view 1038 -R chicago-film-supplies/api-cloudrun`.

> ## ⚠️ STATUS UPDATE 2026-09-17 (latest): step 5 publish built, both consumers pinned, unreleased
> - **core `5bb03f7` → `@cfs/core@10.0.0-beta.484`** (`feat(charge-windows)!`). `OrderDocDates`' `charge_start`/`charge_end`(+`_fs`)/`days_charged` and the four booking `charge_*` keys are `.nullable().optional()`, catalogued `mid-expand` in `tests/stored-optionality.test.ts` (19 paths). `canonicalChargeWindows` DELETES them, so every rewritten pair is purged lazily; `buildRemainingInvoice`'s extension pair no longer carries them; `resolveMergedPairDates` stops carrying their `_fs`; `deriveOrderDateEnvelope` and `ChargeDates` lose the charge half; the `dates.charge_*_fs` / `days_charged` fields (and the `destinations[]` twins) are gone from the `orders`, `fulfillments`, `invoices` and `bookings` Typesense configs with their display-column rollups. The int-parity floors dropped 190→180 and 70→63 with a dated reason.
> - **Built against the local core BEFORE publishing** (`build-the-consumer-against-a-local-core`), in an api-cloudrun worktree with `deno.json` repointed at `file://`. That found six compile errors and, more usefully, two sites no grep would have flagged: `services/invoices.ts` re-stamped `charge_*_fs` from the deleted keys, and `pairEdit`'s `{ ...stored, ...recounted }` spread carried them straight back past the recount.
> - **manager `032f90d8`** (on `main`): pin beta.484, draft seed stops stating the fields, tests assert their absence. 2,163 unit tests green.
> - **api-cloudrun `fa1688d4`** (on `main`, full pre-push suite green against dev): pin beta.484; `canonicalizeDestinationDates`, the invoice pair canonicalizer, `pairEdit` and `buildBookingDates` stop writing them; `xeroQuoteStatus` and its audit-script twin fall back to the latest charge-window end instead of the removed envelope `charge_end`.
> - **Left, all in api-cloudrun#1038:** release manager then the API; purge dev then prod with a before/after baseline; force the prod Typesense resync; then beta C deletes the declarations. Two questions to settle first: `scripts/_stockReconciliationPlan.ts`'s `booking.dates.charge_start` fallback (an ORACLE, not a render), and the templates fixtures that still spell the keys.
> - **Dropping the booking keys makes `bookingWriteIsNoOp` see every stored booking as changed once**, so the next order edit rewrites its bookings. That is the cheap direction (a write that would have happened anyway) and it is how bookings get purged lazily.

> ## ⚠️ STATUS UPDATE 2026-09-17 (latest, compacted): core#114 and the partials in prod; fixture PR open
> - **templates#378** (merged): `destinations.eta` / `items-grid.eta` read `charge_windows`. Single window: `visual-diff` matched all 45 fixtures in 7 families. 2+ windows: one row per window with its days, then **Total Charged in billable days**. `formatChargeDays` throws on 0, so 0-day windows are guarded.
> - **core#114, owner decision: a 2+ window line stores its BILLABLE days** (Σ max(d,5)), so the Duration column agrees with the money. Core `e9cbd48`: `lineChargeableDays` stamps `billableDays`; the pricer's separate window input is gone (`LinePricingOptions.windowDays`, `priceLine`'s 4th argument, `CreditSourceLine.window_days`). This also fixed `priceCreditNote`. Latent: prod has no 2+ window pair. Released as manager 26.26.1, then API 0.277.1.
> - **templates PR #381** (prod MCP draft `0jWvCr7Lv26s3MjLoxhm`): `invoice/multi-charge-window`, built from `rental-discount-taxed` with windows [3,4,2]. The preview renders three window rows, Total Charged 3 weeks, line Duration 3 weeks, and $36.00 + $3.96 = $39.96. Lint is clean except for the missing goldens. Release armed auto-merge and `no-golden` passes, so bless the golden even if it has already merged. The quote fixture waits on templates#343.
> - **Consequences for later steps:** core#113's invariant is "every rental `five_day_week` line on a 2+ window pair has `chargeable_days === billableDays(windows)`". Step 6's multi-window Duration cell shows the stored line days, read-only.
> - api-cloudrun#1002 recurred on the first #378 draft; that draft was abandoned and redone.
