...
## START HERE
**Step 4 (beta B) is in prod as of 2026-09-17**: manager 26.26.0, then API 0.277.0, templates#375 merged. **templates#378 is released** (auto-merge armed, publishes on merge). **core#114 is built** as `@cfs/core@10.0.0-beta.483`, and #378 is merged. Pins on beta.483: manager `c97bd339` (+ test `a4d45be4`), API `3b03d8a0` (both on `main`, not released; release manager before the API), templates PR #380 (open, checks green). Next: the templates#376 multi-window fixture, then the rest of step 5's prerequisites (status block below). First command: `gh pr view 378 -R chicago-film-supplies/templates`.
