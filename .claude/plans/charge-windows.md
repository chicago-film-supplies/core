...
**Date:** 2026-09-16 • **Repo:** core (+ api-cloudrun, manager, templates) • **Status:** in-progress (steps 1–4 + core#114 in prod; templates#381 fixture open; step 5 next)
...
## START HERE
**Steps 1–4 are in prod, and so is core#114** (`@cfs/core@10.0.0-beta.483`): manager 26.26.1, then API 0.277.1 (which also carries the reopen reprice, `cf4b9673`), templates #378 (partials read windows) and #380 (pin) merged. **templates#376's multi-window fixture is PR #381** (`invoice/multi-charge-window`, synthetic, [3,4,2] → Total Charged 3 weeks, $39.96). It needs its golden blessed. **Next: step 5** (stop writing, then purge, then remove), starting from the prerequisites list below. First command: `gh pr view 381 -R chicago-film-supplies/templates`.
