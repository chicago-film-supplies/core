...
- ~~**A display convention must not become a document field.**~~ 🔴 **Reversed on 2026-09-08, and the reversal is the more useful lesson.** The rule read: the owner designation exists because a TABLE must not draw one aggregate booking's quantities on N rows, *a document has no such constraint*, so the wire states the bookings once and every line carries the same id — "*more* truthful than designating an arbitrary owner". The second clause was the load-bearing one and it was a claim about **the documents that existed then**, not about documents. A packing list with a state column per `breakdown` bucket asks per ROW, and the moment it does it inherits the table's constraint exactly: `PickSheetDestination.bookings` answers per LEG and cannot answer per row, so every consumer was left to re-derive the owner rule or double-count — manager already had it (`ownerPathByItemPath`), the template had nothing, and 384 prod legs carry a booking spanning 2+ lines. `PickSheetItem.owner_path` is now a document field and `foldPickSheet` its one author. ⭐ **Ask what the document's CONSUMERS have to compute, not what the document is "about"** — a fact every reader must derive identically is a fact the writer should have stated, and "*this consumer happens not to need it yet*" expires.
- ⚠️ **What survives is narrower and still right: a display convention must not become a document field.** The owner *designation* crossed over; the manager's `Map`s, its whole `Booking` objects and its `stage` label did not. The test is whether every consumer would compute the same value — not whether one consumer finds it convenient.
...
### The line price has ONE author, and it lives here

**`computeLineMoney` + `assembleLinePrice` (`src/utils/orders.ts`) are the one place a stored line's `price` object is decided and assembled** — for the API, the manager and this package alike (api-cloudrun#570). The branch used to be duplicated once per writer: six in `api-cloudrun/src/`, one in a repair script, four in the manager. Teaching the system a `transaction_fee` cost one edit per writer plus a static ratchet purely to guarantee the last one was not forgotten.

**`priceDocument` (`src/utils/price-document.ts`) is the one DOCUMENT author that runs them** (api-cloudrun#997, done 2026-09-14): tax (`assignLineTaxes`, freezing an issued invoice's versions) → per-line `computeLineMoney` → `assembleLinePrice` → fees (a `percent_of_total` fee line **stores** its costed amount) → totals as the **SUM** of stored line money. Every api-cloudrun and manager order/invoice writer calls it; `priceCreditNote` is its credit-note form. ⚠️ **A re-deriving totals function survives only as the audit oracle** (`rederiveDocumentTotalsForAudit`, `rederiveInvoiceTotalsForAudit`) — pointing the #575 check at the sum would test the implementation against itself. `materializeDocumentTax` and the old re-pricing totals functions are DELETED, not wrapped.

- `computeLineMoney(item, taxes, label?)` — the three-arm branch: pre-tax → `calculateItemPrice`, `from_total` → `priceTransactionFeeLine`, anything else → **throws by name**. ⚠️ Not a partition of two: a divider has `pricing: "none"` and a document can carry a type with no contract at all, so a two-arm form silently stores a `$0` line.
- `assembleLinePrice(declared, money, item, { taxesBase })` — **generic in the caller's own price shape**, so an order line keeps `replacement_cents` and an invoice line can never gain one. The spread carries the shape; the function never names it.
- ⚠️ The price object has **three** halves, not two: *declared* (the caller's input), *computed* (the pricer), and **`taxes_base`, which is neither** — it does not exist until the pricer has run, it is not money, and its SOURCE differs per writer (the line's charged taxes for the order writers, the **product's** taxes for the product cascade). It is an explicit input. **Omitting it means "this writer does not author the key"**, which is what keeps the CRMS invoice writer's literal at 8 keys — widening it would read as a replacement on ~9k `isEqual`-diffed lines and re-push every CRMS invoice to Xero.
- `assembleLinePrice` **asserts the money it is handed**: `discount.amount_cents === subtotal_cents − subtotal_discounted_cents` and `total_cents === subtotal_discounted_cents + Σ taxes[].amount_cents`. Two writers supply their own money (CRMS's, Xero's) and between them they account for the whole stored invoice corpus, so on exactly that path the identities `calculateItemPrice` guarantees by construction are unguaranteed. A location ratchet needs a value assertion beside it.
- ⚠️ **A flat fee's stored `subtotal_cents` (`priceTransactionFeeLine`) and its rollup amount (`totals.transaction_fees`) deliberately DISAGREE** — stored is pre-discount, the rollup's is discounted. `xeroLineMoney` reads the stored one as `UnitAmount` and applies `DiscountRate` on top, so collapsing them takes the discount twice on a live ledger with no dev tenant. `tests/orders.test.ts` pins the disagreement.
- Guarded by **Ratchet E** in `tests/moneyArithmeticCoverage.test.ts`: no pre-tax pricer is called outside `computeLineMoney` in `src/`, per file, at an **exact count**. It arrived with the branch — moving code without moving its guard removes one silently.
...
- 🔴 **The order → invoice / fulfillment SHARED-FIELD classification is a different thing from this catalog, and it is declared per FIELD and fails closed.** `classifySharedFields` (`utils/shared-fields.ts`) reads the merge's field set off the two Zod schemas; every shared value needs `.meta({ propagate: true })`, `.meta({ derived: true })` or `.meta({ propagate: false })` on its declaration, and an untagged one lands in `undeclared` and throws at both callers (core#116, 2026-09-18 — it used to default to propagating). **A tag on EITHER schema counts**, so one tag on the order's declaration covers every grain that spreads it. ⚠️ Two different things here are called *propagation*; conflating the rule CATALOG with the shared-field MERGE is a recurring misconception, and the `cfs-order-projections` skill is the authority on the second.
...
- **`column: true` is opt-in.** Before this, a walker enumerated ~375 columns
  ...
  declared columns as of 2026-08-31; `manager/tests/stores/declaredColumns.test.ts`
