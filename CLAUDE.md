...
- `assembleLinePrice(declared, money, item, { taxesBase })` — **generic in the caller's own price shape**, so an order line keeps `replacement_cents` and an invoice line can never gain one. The spread carries the shape; the function never names it. This is `materializeDocumentTax`'s design, one level down.
...
- ⚠️ **`priceTransactionFeeLine` and `costTransactionFees` deliberately DISAGREE about `subtotal_cents`** — stored is pre-discount, the rollup's is discounted — and they now sit ~330 lines apart in one file where two repos used to hold them apart. `xeroLineMoney` reads the stored one as `UnitAmount` and applies `DiscountRate` on top, so collapsing them takes the discount twice on a live ledger with no dev tenant. `tests/orders.test.ts` pins the disagreement.
