...
export const templateHelpers: Record<string, TemplateHelperEntry[]> = {
  ...
  "taxes": [
    ...
    { name: "deriveJurisdiction", expr: "it.taxes.deriveJurisdiction(address, origin)", desc: "**Where a delivery address sources to** — the bottom of the three-level jurisdiction precedence, below a destination's explicit `jurisdiction` and an organization's `default_jurisdiction`.", returns: "JurisdictionType | null" },
    ...
    { name: "isEntirelyOutOfIllinois", expr: "it.taxes.isEntirelyOutOfIllinois(destinations)", desc: "Does **every** destination on this document deliver outside Illinois?", returns: "boolean" },
    ...
    { name: "materializeOrderTax", expr: "it.taxes.materializeOrderTax(items, orgProfile, docProfile, destinations, taxCatalog, now)", desc: "**The one order-side tax materializer.** Resolve an order's effective tax from its organization snapshot, its own `tax_profile` and its destinations, then reprice every billable line. Mutates `items` in place; callers run `calculateOrderTotals` after.", returns: "void" },
    ...
    { name: "resolveEffectiveProfile", expr: "it.taxes.resolveEffectiveProfile(orgProfile, docProfile)", desc: "The **one precedence rule**, as a profile rather than as a resolved Tax: exemption from either side wins, then the document's profile, then the organization's.", returns: "TaxProfileType" },
  ],
  "templates": [
