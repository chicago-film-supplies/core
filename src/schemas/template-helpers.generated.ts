...
export const templateHelpers: Record<string, TemplateHelperEntry[]> = {
  ...
  "invoices": [
    { name: "buildInvoiceDestinationDivider", expr: "it.invoices.buildInvoiceDestinationDivider(source, arg2)", desc: "Build an invoice destination divider from a source order's destination item. Single source of truth for the divider shape — reused by `projectOrderItemToInvoiceItem` (order→invoice projection), the CRMS invoice webhook (`createUpdateInvoiceFromCrms`), and the destination-divider backfill.", returns: "InvoiceItem" },
    { name: "calculateInvoiceTotals", expr: "it.invoices.calculateInvoiceTotals(items, taxes, payments)", desc: "Calculate aggregated pricing totals for an invoice.", returns: "InvoiceTotals" },
    { name: "calculateItemDiscount", expr: "it.invoices.calculateItemDiscount(item)", desc: "Calculate the discount dollar amount for a single line item.", returns: "number" },
    ...
    { name: "calculateItemTotal", expr: "it.invoices.calculateItemTotal(item, taxes)", desc: "Calculate the total (subtotal_discounted + taxes) for a single line item. Handles both PriceObject (regular items) and PriceModifier (transaction fee items).", returns: "number" },
    { name: "derivePaymentStatus", expr: "it.invoices.derivePaymentStatus(currentStatus, amountPaid, amountDue)", desc: "Derive invoice status from payment amounts. Pure function — does not mutate the invoice.", returns: "string" },
    { name: "getOrderScopedItems", expr: "it.invoices.getOrderScopedItems(items, orderDividerUid)", desc: "Get all invoice items scoped to a specific order divider. Returns the order divider itself plus all items whose path starts with the order divider's uid.", returns: "InvoiceItem[]" },
  ...
  "orders": [
    ...
    { name: "calculateItemTotal", expr: "it.orders.calculateItemTotal(item, taxes)", desc: "Calculate the total (subtotal_discounted + taxes) for a single line item. Handles both PriceObject (regular items) and PriceModifier (transaction fee items).", returns: "number" },
    ...
    { name: "getTotalDiscount", expr: "it.orders.getTotalDiscount(items)", desc: "Calculate the total discount amount across all pre-tax items.", returns: "number" },
    { name: "getTransactionFeeTotals", expr: "it.orders.getTransactionFeeTotals(items)", desc: "Aggregate transaction fee PriceModifiers across all fee items.", returns: "PriceModifier[]" },
  ...
  "products": [
    { name: "buildComponentEntries", expr: "it.products.buildComponentEntries(parentUid, sourceComponents, baseDepth, maxDepth)", desc: "Build component entries for a parent product from a component product's own `components` array. Each entry's `path` is prepended with `parentUid` so it reflects its position in the parent's tree.", returns: "ProductComponent[]" },
    { name: "removeComponentEntries", expr: "it.products.removeComponentEntries(components, path)", desc: "Remove a component and all its descendants from a flat components array. An entry is removed if its `path` starts with the given path prefix — this covers the component itself and every entry nested beneath it.", returns: "ProductComponent[]" },
  ],
  "taxes": [
    ...
    { name: "overrideItemTaxesForProfile", expr: "it.taxes.overrideItemTaxesForProfile(items, orgProfile, docProfile, taxCatalog, asOf)", desc: "Materialize a doc-level `tax_profile` override onto each priceable item's `price.taxes` (single mode — mutates in place): - `tax_exempt` → `taxes = []`, `total = subtotal_discounted`. - `tax_rantoul` / `tax_frankfort` → `taxes = [<resolved tax>]` with amount computed from the item's **existing** `subtotal_discounted` + `total` refreshed. (Orders re-run `calculateItemPrice` after, which recomputes both from the rewritten uid; the CRMS invoice webhook keeps the amounts computed here on its `charge_total`-authoritative subtotal.) - `tax_applied` / no active override doc → item left untouched.", returns: "void" },
  ],
  "templates": [
