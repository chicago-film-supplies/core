...
const SELECTIVE_SYNC_SEMANTICS: EnforcementRef = {
  ...
  clause:
    "the override policy, per helper. The automatic per-save sync is PER FIELD: `syncOrderToInvoiceSelective` and `syncOrderDestinationsSelective` merge each shared field of a matched row or pair, taking the order's new value unless the invoice's differs from the order's PREVIOUS one, with derived fields never compared and recomputed afterwards. The field set is read off the two schemas (`orderInvoiceSharedFields`), so there is no list here to drift — and no whole-row or whole-pair freeze: one differing field no longer freezes its siblings. `syncOrderDestinationScope` decides a destination the order deleted as ONE row — its divider and its pair are kept together if either half is overridden, dropped together otherwise (api-cloudrun#664). The operator's hard resync is a DIFFERENT policy and keeps its own: `syncOrderItems` (scoped replace + `carryForwardOverrides`) rebuilds a scope from the order, discarding overrides and dropping removed lines, preserving only the invoice-only fields named by `INVOICE_ONLY_ITEM_FIELDS` — RE-DERIVE that set from the list rather than counting it here, which this clause got wrong three times. Does NOT cover the FREEZE predicate that decides which invoices are eligible.",
  gates: true,
};
...
const updateOrderInvoiceRules: CollectionRule[] = [
  {
    ...
    fields: [
      ...
      },
      {
        ...
        target: ["items"],
        transform:
          "selective sync: compare prev order items to current invoice items by path — update only non-overridden items, add new items, remove deleted non-overridden items. All SEVEN invoice-only fields are preserved: coa_revenue, tracking_category, xero_id, xero_tracking_option_id, crms_id, crms_opportunity_id, path_substituted_for. ⚠️ The seventh is unlike the other six and joined on the substitution work: it is a DIVERGENCE RECORD rather than a value, this sync is its author as well as its preserver, and it is RE-POINTED at X's current order path on every run (api-cloudrun#897) rather than carried forward untouched. ⚠️ This listed FOUR until 2026-08-18 — `INVOICE_ONLY_ITEM_FIELDS` (`core/src/utils/invoices.ts`) has been six since 2026-08-10, and `SELECTIVE_SYNC_SEMANTICS`' clause above was corrected on 2026-08-17 while ASSERTING that this transform had been corrected too. It had not been: the clause and the transform are two descriptions of one rule, and fixing one is not fixing the rule (core#55 item 1, arity half). ✅ Ownership — whether these are invoice-owned at all, given `productLineDenorms` documents itself as their sole author — was the OTHER half, and it is RULED: api-cloudrun#537, Alex 2026-08-18, **point-in-time wins**. This clause is now true of `tracking_category` rather than aspirational: `preserveStoredTrackingCategory` keeps a stored non-null value against the live product on every invoice writer, native and CRMS. ✅ **`coa_revenue` is RULED too — api-cloudrun#580, 2026-08-22 — and it went the OTHER way, as a per-line OVERRIDE rather than as point-in-time preservation.** An operator sets the revenue account on a line and it wins over the product's; `preserveStoredCoaRevenue` (api-cloudrun `lib/invoiceLineDenorms.ts`) then carries it across every rebuild. ⚠️ The two rulings differ because the AUTHORSHIP differs, not because the fields do: nothing but the product authors a line's tracking category, so a stored value there is a historical record, while `coa_revenue` is accepted on `InvoiceItemInputLine` and is chosen. ⚠️ And the tax half of that old warning is GONE — api-cloudrun#596 deleted `isTaxableCoa` and `TAXABLE_REVENUE_COAS`, so the account no longer moves Xero's tax treatment and is purely an `AccountCode`. The remaining three (`xero_id`, `xero_tracking_option_id`, `crms_id`) are still resolved from the product on every write and are NOT ruled on.",
      },
      {
        source: ["items"],
        target: ["totals"],
      ...
      },
      {
        ...
        target: ["destinations"],
        transform:
          "selective sync within uid_order scope: match pairs by pair.uid (the destination divider's uid — NOT the endpoint uids, which move when an address is corrected: api-cloudrun#663) — merge each matched pair PER FIELD, add new pairs (tagged with uid_order), remove deleted pairs no field of which was overridden. Leaves pairs from other orders untouched. A pair is one row with its destination divider (`pair.uid` is the last segment of the divider's path): when the order deletes a destination, `syncOrderDestinationScope` keeps divider AND pair if either half is overridden, and drops both otherwise, so the two halves can never be split (api-cloudrun#664). Both halves run the same per-field test, so `jurisdiction` is an ordinary shared field: editing it on the invoice IS an override and now KEEPS a pair the order deleted — the reverse of the pre-v0.270.0 rule, where it was invoice-owned, skipped by the comparison, and the pair was dropped.",
      },
      {
        source: ["subject"],
        target: ["subject"],
